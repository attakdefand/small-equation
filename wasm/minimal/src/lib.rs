...
fn cumulative_normal(x: f64) -> f64 {
    (1.0 + libm::erf(x / libm::sqrt(2.0))) / 2.0
}

/// Probability density function for standard normal distribution
fn normal_pdf(x: f64) -> f64 {
    (1.0 / libm::sqrt(2.0 * PI)) * libm::exp(-0.5 * x * x)
}
...
fn black_scholes_call(
    ...
    let d1 = (libm::log(s / k) + (r + 0.5 * sigma * sigma) * t) / (sigma * libm::sqrt(t));
    let d2 = d1 - sigma * libm::sqrt(t);
    
    s * cumulative_normal(d1) - k * libm::exp(-r * t) * cumulative_normal(d2)
}
...
fn black_scholes_put(
    ...
    k * libm::exp(-r * t) * cumulative_normal(-d2) - s * cumulative_normal(-d1)
}
...
fn gamma(
    ...
    let d1 = (libm::log(s / k) + (r + 0.5 * sigma * sigma) * t) / (sigma * libm::sqrt(t));
    normal_pdf(d1) / (s * sigma * libm::sqrt(t))
}
...
fn theta_call(
    ...
    - (s * normal_pdf(d1) * sigma) / (2.0 * libm::sqrt(t)) 
    - r * k * libm::exp(-r * t) * cumulative_normal(d2)
}
...
fn rho_call(
    ...
    k * t * libm::exp(-r * t) * cumulative_normal(d2)
}
